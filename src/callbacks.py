...
from .utils import softmax, coco_evaluation, create_annotations, make_apply_transformer, get_device
...
class ValidationMonitorSegmentation(ValidationMonitor):
    ...
    def _get_validation_loss(self):
        with TemporaryDirectory() as temp_dir:
            ...
            if len(prediction) == 0:
                return self.validation_loss.setdefault(
                    self.epoch_id, {'sum': Variable(torch.Tensor([0]))})
            ...
            annotation_file_path = os.path.join(self.data_dir, 'val',
                                                "annotation.json")
        ...
        return self.validation_loss.setdefault(
            self.epoch_id,
            {'sum': Variable(torch.Tensor([average_precision]))})
